...
![QQ截图20230408191839.png](/img/12-Sequencer/QQ截图20230408191839.png)

```64位数字ID：我们将长度限制为64位，因为这个位数足够未来的许多年。让我们计算一下我们的ID范围将在多少年后循环。可用总数=每天的预计事件数=10亿，每年的事件数=3650亿，用尽标识符范围的年数=根据这些计算来说，64位应该足以成为唯一ID长度。```
让我们深入讨论上述问题的可能解决方案。

## 第一种解决方案：UUID

我们设计的第一个稻草人解决方案使用UUID（通用唯一标识符）。这是一个128位的数字，看起来像456426614174000123e4567e89b12d3a456426614174000（16进制）。它给了我们大约$2^{128}$个数。UUID有不同的版本。我们选择版本4，它生成伪随机数。

![QQ截图20230408192758.png](/img/12-Sequencer/QQ截图20230408192758.png)
...
### 优点这个系统是可伸缩的，可用的，并且产生没有重复的用户ID。此外，我们可以在64位中维护此范围，这是数字化的。
