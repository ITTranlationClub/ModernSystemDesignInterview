...
const _imports_0 = "/model-system-design/img/07-DomainNameSystem/QQ截图20230406205130.png";
const _imports_1 = "/model-system-design/img/07-DomainNameSystem/QQ截图20230406205217.png";
const _imports_2 = "/model-system-design/img/07-DomainNameSystem/QQ截图20230413204931.png";
const _imports_3 = "/model-system-design/img/07-DomainNameSystem/QQ截图20230413204948.png";
const _imports_4 = "/model-system-design/img/07-DomainNameSystem/QQ截图20230413205001.png";
const _imports_5 = "/model-system-design/img/07-DomainNameSystem/QQ截图20230413205012.png";
const _imports_6 = "/model-system-design/img/07-DomainNameSystem/QQ截图20230413205028.png";
const _imports_7 = "/model-system-design/img/07-DomainNameSystem/QQ截图20230413205043.png";
const _imports_8 = "/model-system-design/img/07-DomainNameSystem/QQ截图20230413205110.png";
const _imports_9 = "/model-system-design/img/07-DomainNameSystem/QQ截图20230406205439.png";
const _imports_10 = "/model-system-design/img/07-DomainNameSystem/QQ截图20230406205451.png";
const _sfc_main = {};
const _hoisted_1 = /* @__PURE__ */ createStaticVNode('<p>了解域名系统的详细工作原理。</p><p>通过本课程，我们将回答以下问题：</p><ul><li>如何使用不同类型的DNS名称服务器形成DNS层次结构？</li><li>在互联网的不同级别上如何进行缓存以减少DNS基础设施的查询负担？</li><li>DNS基础设施的分布式本质如何帮助其健壮性？</li></ul><p>让我们开始吧。</p><h2 id="dns层次结构" tabindex="-1"><a class="header-anchor" href="#dns层次结构" aria-hidden="true">#</a> DNS层次结构</h2><p>如前所述，DNS不是单个服务器，它接受请求并响应用户查询。它是一个完整的基础设施，具有不同层次的名称服务器。</p><p>DNS层次结构中主要有四种类型的服务器：</p><ol><li><p><strong>DNS解析器：</strong> 解析器启动查询序列并将请求转发给其他DNS名称服务器。通常，DNS解析器位于用户网络的范围内。但是，DNS解析器还可以通过缓存技术响应用户的DNS查询，稍后我们将详细介绍。这些服务器也可以称为本地或默认服务器。</p></li><li><p><strong>根级别名称服务器：</strong> 这些服务器收到本地服务器的请求。根名称服务器基于顶级域名（如<code>.com</code>、<code>.edu</code>、<code>.us</code>等）维护名称服务器。例如，当用户请求<code>educative.io</code>的IP地址时，根级别名称服务器将返回一个包含<code>.io</code>域的IP地址的顶级域名（TLD）服务器列表。</p></li><li><p><strong>顶级域名（TLD）名称服务器：</strong> 这些服务器持有授权名称服务器的IP地址。查询方将获取属于组织的授权服务器的IP地址列表。</p></li><li><p><strong>授权名称服务器：</strong> 这些是组织的DNS名称服务器，它们提供Web或应用程序服务器的IP地址。</p></li></ol><p><img src="' + _imports_0 + '" alt="DNS层次结构以解析域名/主机名" loading="lazy"></p>', 9);
const _hoisted_10 = /* @__PURE__ */ createStaticVNode('<div class="hint-container info"><p class="hint-container-title">相关信息</p><h6 id="问题" tabindex="-1"><a class="header-anchor" href="#问题" aria-hidden="true">#</a> 问题</h6><p>DNS名称如何处理？例如，<code>educative.io</code>将从左到右或从右到左处理？</p><p>答案:</p><p>与UNIX文件不同，DNS名称从右到左处理。对于<code>educative.io</code>，解析器将首先解析<code>.io</code>部分，然后是<code>educative</code>等等。</p><p>视觉上，DNS层次结构可以看作是一棵树。</p></div><h3 id="迭代查询和递归查询解决方案比较" tabindex="-1"><a class="header-anchor" href="#迭代查询和递归查询解决方案比较" aria-hidden="true">#</a> 迭代查询和递归查询解决方案比较</h3><p>有两种方法可以执行DNS查询：</p><ol><li><p><strong>迭代：</strong> 本地服务器向根服务器、TLD服务器和授权服务器请求IP地址。</p></li><li><p><strong>递归：</strong> 终端用户向本地服务器请求。本地服务器进一步请求DNS根名称服务器。根名称服务器将请求转发给其他名称服务器。</p></li></ol><p>在下面的插图（图左）中，本地/ISP服务器的DNS查询解决方案是迭代的：</p><p><img src="' + _imports_1 + '" alt="递归查询和迭代查询" loading="lazy"></p>', 6);
const _hoisted_16 = /* @__PURE__ */ createStaticVNode('<div class="hint-container info"><p class="hint-container-title">相关信息</p><p><strong>注意：</strong> 通常，迭代查询优先于递归查询，以减少DNS基础设施的查询负载。</p></div><p>现在，我们会发现许多谷歌、Cloudflare、OpenDNS等提供的第三方公共DNS解析器。有趣的事实是这些公共DNS服务器可能会比本地ISP DNS设施提供更快的响应。</p><h2 id="缓存" tabindex="-1"><a class="header-anchor" href="#缓存" aria-hidden="true">#</a> 缓存</h2><p><strong>缓存</strong>指的是经常请求的<code>资源记录</code>的临时存储。</p><p>一个<strong>记录</strong>是DNS数据库中显示名称与值绑定的数据单元。</p><p>缓存减少了对用户的响应时间并减少了网络流量。</p><p>当我们在不同的层次使用缓存时，它可以减少DNS基础架构上的许多查询负担。缓存可以在浏览器、操作系统、用户网络内的本地名称服务器或ISP的DNS解析器中实现。</p><p>下面的幻灯片演示了DNS中缓存的威力：</p><p><img src="' + _imports_2 + '" alt="QQ截图20230413204931" loading="lazy"></p><p>用户请求访问一个URL，浏览器已经缓存了域名到IP地址的映射</p><p><img src="' + _imports_3 + '" alt="QQ截图20230413204948" loading="lazy"></p><p>如果浏览器没有缓存域到IP地址的映射，接下来可以有该映射的OS</p><p><img src="' + _imports_4 + '" alt="QQ截图20230413205001" loading="lazy"></p><p>如果OS没有该映射，本地DNS解析器可以有缓存响应</p><p><img src="' + _imports_5 + '" alt="QQ截图20230413205012" loading="lazy"></p><p>如果本地DNS解析器没有该映射，ISP可以有缓存响应</p><p><img src="' + _imports_6 + '" alt="QQ截图20230413205028" loading="lazy"></p><p>最后，DNS基础架构将以IP响应</p><p><img src="' + _imports_7 + '" alt="QQ截图20230413205043" loading="lazy"></p><p>缓存将在每个层次更新</p><p><img src="' + _imports_8 + '" alt="QQ截图20230413205110" loading="lazy"></p><p>浏览器现在更新了缓存，所以用户请求将在本地得到服务</p><div class="hint-container tip"><p class="hint-container-title">提示</p><p><strong>注意：</strong> 即使没有可用的缓存来解析用户的查询并且访问DNS基础设施至关重要，缓存仍然有益。本地服务器或ISP DNS解析器可以缓存TLD服务器或权威服务器的IP地址并避免请求根级服务器。</p></div><h2 id="dns作为分布式系统" tabindex="-1"><a class="header-anchor" href="#dns作为分布式系统" aria-hidden="true">#</a> DNS作为分布式系统</h2><p>尽管DNS层次结构促进了我们今天所知的分布式互联网，但它本身是一个分布式系统。 DNS的分布式特性具有以下优点：</p><ul><li><p>避免成为单点故障（SPOF）。</p></li><li><p>实现低查询延迟，以便用户可以从附近的服务器获得响应。</p></li><li><p>在维护、更新或升级时获得更高的灵活性。</p></li></ul><p>例如，如果一个DNS服务器宕机或负担过重，另一个DNS服务器可以响应用户查询。</p><div class="hint-container info"><p class="hint-container-title">相关信息</p><p>有13个逻辑根名称服务器（以字母<strong>A</strong>到<strong>M</strong>命名）在全球范围内分布许多实例。这些服务器由12个不同的组织管理。</p></div><p>现在，让我们来看看DNS如何具有可伸缩性、可靠性和一致性。</p><h3 id="高度可伸缩性" tabindex="-1"><a class="header-anchor" href="#高度可伸缩性" aria-hidden="true">#</a> 高度可伸缩性</h3><p>由于其层次结构性质，DNS是一个高度可伸缩的系统。大约有1,000个复制的13个根级服务器实例在全球战略地分布，以处理用户查询。工作任务分配在TLD和根服务器之间以处理查询，最后，由组织本身管理的权威服务器使整个系统工作。如上所示的DNS层次树中，不同的服务处理不同部分的树，实现了系统的可扩展性和可管理性。</p><h3 id="可靠性" tabindex="-1"><a class="header-anchor" href="#可靠性" aria-hidden="true">#</a> 可靠性</h3><p>DNS成为一个可靠的系统有三个主要原因：</p><ol><li><strong>缓存：</strong> 缓存在浏览器、操作系统和本地名称服务器中进行，ISP DNS解析器也维护着访问频率很高的服务的丰富缓存。即使一些DNS服务器暂时关闭，缓存记录也可以提供服务，从而使DNS成为一个可靠的系统。</li><li><strong>服务器复制：</strong> DNS在全球范围内有每个逻辑服务器的复制副本，以在低延迟下处理用户请求。冗余服务器提高了整个系统的可靠性。</li><li><strong>协议：</strong> 尽管许多客户端使用DNS通过不可靠的用户数据报协议（UDP），但UDP具有其优点。UDP要快得多，因此提高了DNS的性能。此外，自其成立以来，Internet服务的可靠性已经得到了改善，因此通常情况下UDP优于TCP。如果之前的UDP请求没有得到响应，DNS解析器可以重新发送UDP请求。这种请求-响应只需要一次往返，这比TCP需要三次握手更短的延迟。</li></ol><div class="hint-container info"><p class="hint-container-title">相关信息</p><p>问题:</p><p>如果网络拥塞会发生什么？DNS应该继续使用UDP吗？</p><p>答案:</p><p>通常，DNS使用UDP。然而，当其消息大小超过512个字节的原始数据包大小时，DNS可以使用TCP。这是因为大尺寸数据包更容易在拥塞的网络中受到损坏。DNS总是使用TCP进行区域传输。一些客户端喜欢使用DNS over TCP来使用传输层安全性，出于隐私方面的考虑。</p></div><h3 id="一致性" tabindex="-1"><a class="header-anchor" href="#一致性" aria-hidden="true">#</a> 一致性</h3><p>DNS使用各种协议在分层复制服务器之间更新和传输信息。</p><p>为了实现高性能，DNS在强一致性上做出了让步，因为与写入相比，数据经常从DNS数据库中读取。</p><p>然而，DNS提供最终一致性，并懒惰地在复制服务器上更新记录。</p><p>通常情况下，更新DNS服务器上的记录需要几秒钟到三天的时间。</p><p>将信息传播到不同的DNS集群中所需的时间取决于DNS基础架构、更新大小以及正在更新的DNS树的哪个部分。</p><p>由于缓存，一致性可能会受到影响。</p><p>由于权威服务器位于组织内部，因此可能会在组织服务器故障时在权威服务器上更新某些资源记录。</p><p>因此，在默认/本地和ISP服务器上缓存的记录可能已过时。为了缓解这个问题，每个缓存记录都有一个过期时间，称为<strong>生存时间（TTL）</strong>。</p><div class="hint-container info"><p class="hint-container-title">相关信息</p><h6 id="问题-1" tabindex="-1"><a class="header-anchor" href="#问题-1" aria-hidden="true">#</a> 问题</h6><p>为了保持高可用性，TTL值应该是大还是小？</p><p>答案:</p><p>为了保持高可用性，TTL值应该是小的。这是因为如果任何服务器或集群发生故障，组织可以立即更新其资源记录。</p><p>用户将仅在TTL未过期的时间内体验到不可用性。但是，如果TTL很大，组织将更新其资源记录，而用户将继续ping已经崩溃了很久的过时服务器。</p><p>渴望高可用性的公司将TTL值保持在120秒左右。因此，即使发生故障，最大停机时间也是几分钟。</p></div><h2 id="测试一下" tabindex="-1"><a class="header-anchor" href="#测试一下" aria-hidden="true">#</a> 测试一下</h2><p>让我们运行一些命令。单击终端以执行以下命令。将以下命令复制到终端中以运行它们。研究命令的输出：</p><ol><li><code>nslookup www.google.com</code></li><li><code>dig www.google.com</code></li></ol><p>以下幻灯片展示了<code>nslookup</code>和<code>dig</code>输出的一些重要方面。</p><p><img src="' + _imports_9 + '" alt="QQ截图20230406205439" loading="lazy"></p>', 50);
const _hoisted_66 = {
  href: "http://www.google.xn--com-4d2ej48nzo5a/",
  target: "_blank",
...
function _sfc_render(_ctx, _cache) {
  ...
  return openBlock(), createElementBlock("div", null, [
    ...
    createVNode(_component_center, null, {
      default: withCtx(() => [
        createTextVNode("域名/主机名解析的DNS层次结构")
      ]),
      _: 1
      /* STABLE */
    }),
    ...
    createBaseVNode("p", null, [
      createTextVNode("nslookup "),
      createBaseVNode("a", _hoisted_66, [
        createTextVNode("www.google.com的输出"),
...
const _02DomaimNameSystemWorks_html = /* @__PURE__ */ _export_sfc(_sfc_main, [["render", _sfc_render], ["__file", "02DomaimNameSystemWorks.html.vue"]]);
export {
  _02DomaimNameSystemWorks_html as default
};
